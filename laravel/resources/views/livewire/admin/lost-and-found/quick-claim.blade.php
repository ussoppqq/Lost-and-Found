<div class="fixed inset-0 z-[60] overflow-y-auto">
    <!-- Backdrop - JANGAN close modal saat klik backdrop -->
    <div class="fixed inset-0 bg-opacity-60 backdrop-blur-sm transition-opacity"></div>

    <!-- Modal Container -->
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl my-8 max-h-[90vh] overflow-hidden" 
             @click.stop>
            ...
            <!-- Body -->
            <form wire:submit.prevent="processClaim">
                <div class="overflow-y-auto px-6 py-6 space-y-6" style="max-height: calc(90vh - 180px);">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="bg-white rounded-lg p-4 border-2 border-red-200">
                                <div class="flex items-center mb-2">
                                    <span class="px-2 py-1 bg-gray-800 text-white text-xs font-bold rounded mr-2">
                                        {{ $lostReport->formatted_report_number }}
                                    </span>
                                    <span class="px-2 py-1 bg-red-600 text-white text-xs font-bold rounded">LOST</span>
                                </div>
                                <h5 class="font-bold text-gray-900 mb-1">{{ $lostReport->item_name }}</h5>
                                <p class="text-sm text-gray-600 mb-2">{{ Str::limit($lostReport->report_description, 80) }}</p>
                                <div class="flex items-center text-xs text-gray-500">
                                    <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                    </svg>
                                    {{ $lostReport->user->full_name ?? $lostReport->reporter_name }}
                                </div>
                            </div>
                            ...
                            <div class="bg-white rounded-lg p-4 border-2 border-green-200">
                                <div class="flex items-center mb-2">
                                    <span class="px-2 py-1 bg-gray-800 text-white text-xs font-bold rounded mr-2">
                                        {{ $foundReport->formatted_report_number }}
                                    </span>
                                    <span class="px-2 py-1 bg-green-600 text-white text-xs font-bold rounded">FOUND</span>
                                </div>
                                <h5 class="font-bold text-gray-900 mb-1">{{ $foundReport->item_name }}</h5>
                                <p class="text-sm text-gray-600 mb-2">{{ Str::limit($foundReport->report_description, 80) }}</p>
                                <div class="flex items-center text-xs text-gray-500">
                                    <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                    </svg>
                                    {{ $foundReport->user->full_name ?? $foundReport->reporter_name }}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-900 mb-2">Brand / Model</label>
                            <input 
                                type="text" 
                                wire:model="brand"
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-900 mb-2">Color</label>
                            <input 
                                type="text" 
                                wire:model="color"
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                            Verification Photos <span class="text-gray-500 font-normal">(Optional but recommended)</span>
                        </label>
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-purple-500 transition">
                            <input 
                                ...
                                accept="image/*"
                                class="hidden"
                                id="claim-photos-{{ $uploadKey }}">
                            <label for="claim-photos-{{ $uploadKey }}" class="cursor-pointer">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-yellow-700">
                                    By processing this claim, you confirm that the item has been verified and is being returned to the rightful owner. 
                                    This action will create a confirmed match and immediately release the item.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end gap-3 sticky bottom-0">
                    ...
                    <button 
                        ...
                        class="px-5 py-2.5 text-sm font-semibold text-white bg-purple-600 rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <span wire:loading.remove wire:target="processClaim">Process Claim</span>
                        <span wire:loading wire:target="processClaim">Processing...</span>
                    </button>
                </div>
            </form>
        </div>
